...
fn looks_like_marker(text: &str) -> bool {
    ...
    let alphanumeric = |t: &str| {
        t.chars().any(char::is_alphabetic) && t.chars().any(|c: char| c.is_ascii_digit())
    };
...
mod tests {
    ...
    fn paired_machine_markers_are_preserved_by_default() {
        ...
        assert!(c.should_preserve("// prettier-ignore-start"));
        assert!(c.should_preserve("// region:imports"));
    }
